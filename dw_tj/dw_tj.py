...
class DwTj(DwTjBase) :
    ...
    def getUploadId(self, file_path):
        if os.path.isfile(file_path):
            ...
            logging.error(file_path, " doesn't exist, no md5")
            return ""
    ...
    def train(self, dataset_id, modal_type, task_type, scene=1, primary_label=""):
        """
        模型训练
        ...
        projData["primary_main_time_col"] = ""
        ...
        logging.info("createProj", resp)
        if "ret" in resp["data"] and resp["data"]["ret"] != 1:
            return -1, -1, resp
        ...
        logging.info("projTrainResult", resp)
        if "ret" in resp["data"] and resp["data"]["ret"] != 1:
            return -1, -1, resp
